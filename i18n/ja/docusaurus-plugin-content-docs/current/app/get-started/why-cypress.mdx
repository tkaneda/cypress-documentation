...
[Cypressをインストール](/app/get-started/install-cypress)すれば数分で
[エンドツーエンドテスト](/app/end-to-end-testing/writing-your-first-end-to-end-test)あるいは
[コンポーネントテスト](/app/component-testing/get-started)の最初のテストをパスするのを迅速に確認することができる。
...
Cypressを使うとテストを素早く簡単に始めることができる。アプリのテストを開始すると、**ベストプラクティスを使っているのか、スケーラブルな戦略を使っているのか、疑問に思うことがよくある**。

{/* prettier-ignore */}
この方法をガイドするために、Cypressチームは
<Icon name="github" inline="true" contentType="rwa" />を作成した。これは、**実用的で現実的なシナリオでCypressを使う**テストをデモンストレートするフルスタックのサンプルアプリケーションである。

RWAは
[複数のブラウザにわたる](/app/guides/cross-browser-testing)エンドツーエンドテストと
[デバイスサイズ](/api/commands/viewport)を使って完全な
[コードカバレッジ](/app/tooling/code-coverage)を達成するだけでなく、
[視覚的なリグレッションテスト](/app/tooling/visual-testing)、APIテスト、ユニットテストも含め、それらすべてを
[効率的なCIパイプライン](https://cloud.cypress.io/projects/7s5okt)で実行する。RWAを使って、Cypressを使ったウェブアプリケーションテストを**学習し、実験し、いじって、実践してほしい**。

このアプリには必要なものすべてがバンドルされているので、
[リポジトリをクローンするだけ](https://github.com/cypress-io/cypress-realworld-app)で、テストを開始できる。
